...
struct otherNodeProp {
    PartitionID partitionIndex; 
};

struct otherEdgeProp {
...
private:
    ...
    void start_construction(NodeID n, EdgeID m) {
        ...
        m_other_node_props.resize(n);
        m_edges.resize(m);
        m_other_edge_props.resize(m);
    ...
    void finish_construction() {
        ...
        m_other_node_props.resize(node+1);

        m_edges.resize(e);
        m_other_edge_props.resize(e);
    ...
    std::vector<otherNodeProp>  m_other_node_props;
    std::vector<otherEdgeProp>  m_other_edge_props;
...
#ifdef NDEBUG
        return graphref->m_other_node_props[node].partitionIndex;
#else
        return graphref->m_other_node_props.at(node).partitionIndex;
#endif
}

inline void graph_access::setPartitionIndex(NodeID node, PartitionID id) {
#ifdef NDEBUG
        graphref->m_other_node_props[node].partitionIndex = id;
#else
        graphref->m_other_node_props.at(node).partitionIndex = id;
#endif
}
...
#ifdef NDEBUG
        return graphref->m_other_edge_props[edge].rating;        
#else
        return graphref->m_other_edge_props.at(edge).rating;        
#endif
}

inline void graph_access::setEdgeRating(EdgeID edge, EdgeRatingType rating){
#ifdef NDEBUG
        graphref->m_other_edge_props[edge].rating = rating;
#else
        graphref->m_other_edge_props.at(edge).rating = rating;
#endif
}
