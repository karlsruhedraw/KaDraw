...
void gpa_matching::match(const Config & config, 
        ...
        init(G, config, permutation, edge_matching, edge_permutation, sources);

        //permutation of the edges for random tie breaking
        if(config.edge_rating_tiebreaking) {
        ...
        forall_edges(G, e) {
                ...
                if(G.getNodeWeight(source) + G.getNodeWeight(target) > config.max_vertex_weight) {
                        continue;
                }
