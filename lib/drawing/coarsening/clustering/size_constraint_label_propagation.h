...
class size_constraint_label_propagation : public matching {
        public:
                ...
                void remap_cluster_ids(const Config & config, 
                                graph_access & G,
                                std::vector<NodeID> & cluster_id, 
                                NodeID & no_of_coarse_vertices,
                                bool apply_to_graph = true); 

                void create_coarsemapping(const Config & config, 
                                graph_access & G,
                                std::vector<NodeID> & cluster_id, 
                                CoarseMapping & coarse_mapping); 

                void label_propagation(const Config & config, 
                                graph_access & G,
