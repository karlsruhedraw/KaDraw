...
edge_ratings::edge_ratings(const Config & _config) : config(_config){
...
void edge_ratings::rate(graph_access & G, unsigned level) {
        ...
        //if(config.matching_type != MATCHING_RANDOM) {
                //switch(config.edge_rating) {
                        //case EXPANSIONSTAR:
...
void edge_ratings::compute_algdist(graph_access & G, std::vector<double> & dist) {
        for( unsigned R = 0; R < 3; R++) {
                std::vector<double> prev(G.number_of_nodes(), 0);
                forall_nodes(G, node) {
                        prev[node] = random_functions::nextDouble(-0.5,0.5); 
                } endfor

                std::vector<double> next(G.number_of_nodes(), 0);
                double w = 0.5;
...
void edge_ratings::rate_expansion_star_2_algdist(graph_access & G) {
        ...
        std::vector<double> dist(G.number_of_edges(), 0);
